 ...
 * @version 1.0.
 */
 /** Added script to save the Journal Entry for the other account types
 * Client Script - Splunk_Class&Location_Mandatory
 * @author Anchana SV
 ...
 * @version 1.1.
 */
 /** To show the validation message ( of D/C/L) on the save of a Journal Entry rather than adding line items. This requires the change in the script to *show the validation message on Save (rather than adding lines).AS per the requirement just removed validation function declaration in the client *script.
	...
	if(((rectype == 'purchaseorder') && (custform == 116)) || ((rectype == 'vendorbill') && (custform == 124))) {//check whether record type is Purchase Order or Vendor bill
		...
		for(var j = 1; j<= jecount; j++) {
			...
			if (y > -1) {//Checking if the Account type ID is present or not in the Parameters.
				...
				return true;//added for DFCT0050491
		}
