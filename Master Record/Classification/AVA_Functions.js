...
/******************************************************************************************************/
{
	...
	var AVA_UDF1, AVA_UDF2, AVA_EntityUseCode, AVA_ItemAccount, AVA_DefaultShippingCode, AVA_CustomerCode, AVA_ShowMessages, AVA_EnableUseTax, AVA_VendorCode, AVA_GlAccounts, AVA_UseTaxCredit, AVA_UseTaxDebit;
	...
	var TaxCodeStatus, VerifyCredentials, AVA_LineChanged, AVA_EditionChecked = 0;
	...
	var AVA_ClientAtt = 'NetSuite OneWorld 3.4', AVA_DelMax = 200, AVA_DocNo;
...
function AVA_CanadianTaxCalc(GetTaxResult)
{
	var DetailNodes = nlapiSelectNodes( GetTaxResult, "./*[name()='TaxDetail']");
...
function AVA_GetTaxBody(AVA_DocType)
{
	...
	soap = '\t<soap:Body>\n';
		soap += '\t\t<GetTax xmlns="http://avatax.avalara.com/services">\n';
			soap += '\t\t\t<GetTaxRequest>\n';
				soap += '\t\t\t\t<CompanyCode><![CDATA[' + nlapiGetFieldValue('subsidiary') + ']]></CompanyCode>\n';
				...
				if((nlapiGetField('taxitem') == null && nlapiGetFieldValue('custbody_ava_customertaxable') != 'T') || (nlapiGetFieldValue('istaxable') != null && nlapiGetField('taxitem') != null && (nlapiGetFieldValue('istaxable').length == 0 || nlapiGetFieldValue('istaxable') == 'F' || nlapiGetFieldValue('istaxable') == 'No')))
				{
				...
				if(TaxCode1 != null && TaxCode1.substr((TaxCode1.length - 3), 3) != 'POS' && AVA_LocationPOS == 0 && LocationPOS == 0)
				{
					...
					if(TaxCode1 == '-Not Taxable-' && AVA_DefaultTaxCode != null && AVA_DefaultTaxCode.substr((AVA_DefaultTaxCode.length - 3), 3) == 'POS')
					{
				...
				{
					soap += '\t\t\t\t<ReferenceCode><![CDATA[' + (AVA_DocNo != null ? AVA_DocNo.substring(0, 49) : '') + ']]></ReferenceCode>\n';
				}
				...
				if(nlapiGetFieldValue('currencysymbol') != null && nlapiGetFieldValue('currencysymbol').length > 0)
				{
					...
					{
						soap += '\t\t\t\t<CurrencyCode>JPY</CurrencyCode>\n';
					}
					else
					{
				...
				{
					soap += '\t\t\t\t<BusinessIdentificationNo><![CDATA[' + nlapiGetFieldValue('custbody_ava_vatbusinessid').substring(0, 24) + ']]></BusinessIdentificationNo>\n';
				}
			soap += '\t\t\t</GetTaxRequest>\n'; 
		soap += '\t\t</GetTax>\n';
	soap += '\t</soap:Body>\n';
...
//			}		
	...
	if(nlapiGetFieldValue('ismultishipto') != null && nlapiGetFieldValue('ismultishipto').length > 0 && nlapiGetFieldValue('ismultishipto') != 'F' && nlapiGetFieldValue('ismultishipto') != 'No' && AVA_LocationPOS != 1)
	{
...
function AVA_EvaluateAddressInfo(AddressInfo) 
{
	...
	var bOption1 = (((AddressInfo[0] != null && AddressInfo[0].length > 0) || (AddressInfo[0] != null && AddressInfo[0].length > 0)) && (AddressInfo[6] != null && AddressInfo[6].length > 0)) ? true : false;
	var bOption2 = (((AddressInfo[0] != null && AddressInfo[0].length > 0) || (AddressInfo[0] != null && AddressInfo[0].length > 0)) && (AddressInfo[4] != null && AddressInfo[4].length > 0) && (AddressInfo[5] != null && AddressInfo[5].length > 0)) ? true : false;
		...
		if(AVA_ServiceTypes != null && AVA_ServiceTypes.search('TaxSvc') != -1)
		{
			...
			if(DocTaxCode == 'T')
			{
				...
				if((type == 'create' || type == 'edit' || type == 'copy'))
				{
					...
					form.addField('custpage_ava_customsubsidiaryinfo', 'longtext', 'Custom Subsidiary Info');
					form.getField('custpage_ava_customsubsidiaryinfo').setDefaultValue('');
					form.getField('custpage_ava_customsubsidiaryinfo').setDisplayType('hidden');
					
					var cols = new Array();
					cols[0] = new nlobjSearchColumn('custrecord_ava_subdeftaxcode');
					cols[1] = new nlobjSearchColumn('custrecord_ava_iscompanyaddr');
					cols[2] = new nlobjSearchColumn('custrecord_ava_companyaddr');
					cols[3] = new nlobjSearchColumn('custrecord_ava_shipaddr');
					cols[4] = new nlobjSearchColumn('custrecord_ava_subsidiary');
							
					var searchResult = nlapiSearchRecord('customrecord_avasubsidiaries', null, null, cols);
					if(searchResult != null && searchResult.length > 0)
					{
						form.getField('custpage_ava_customsubsidiaryinfo').setDefaultValue(JSON.stringify(searchResult));
					}
				}
						...
						for(var i=0; searchresult != null && i < searchresult.length; i++)
						{
							...
							if(nlapiGetRecordId() == recordid)
							{
								...
								else
								{
									...
									URL1 = URL1 + '&doctype=' + AVA_RecordType() +'&doccode=' + nlapiGetRecordId() + '&rectype=' + nlapiGetRecordType() + '&subid=' + searchresult[i].getValue('custrecord_ava_subsidiaryid');
		...
		{   
			if ((nlapiGetFieldValue('istaxable') == 'T' || nlapiGetFieldValue('istaxable') == 'Yes') || (nlapiGetFieldValue('custpage_ava_formtaxcode') != null && nlapiGetFieldValue('custpage_ava_formtaxcode').length > 0))
			{
				if(AVA_EditionChecked == 2)
				{
					...
					if(FormTaxCode.search('POD') != -1 || FormTaxCode.search('POS') != -1)
					{
			...
			if ((nlapiGetFieldValue('istaxable') == 'F' || nlapiGetFieldValue('istaxable') == 'No') && ((nlapiGetFieldValue('custpage_ava_formtaxcode') != null && nlapiGetFieldValue('custpage_ava_formtaxcode').length > 0 && nlapiGetFieldValue('custpage_ava_formtaxcode').substring(0, TaxCode.length) != TaxCode) || (nlapiGetFieldValue('custpage_ava_formtaxcode') != null && nlapiGetFieldValue('custpage_ava_formtaxcode').length <= 0)))
			{
				AVA_ErrorCode = 6;
				return 1;
			}		
		...
		for(var line = 0 ; AVA_NS_Lines != null && line < AVA_NS_Lines.length ; line++)
		{
			...
			if(LineTaxCode != '-Not Taxable-' && !(linetype == 'Description' || linetype == 'Subtotal' || linetype == 'Group' || linetype == 'EndGroup')) 
			{
				if(LineTaxCode != null && LineTaxCode.length > 0)
				{
					...
					{
						if(LineTaxCode.search('POD') != -1 || LineTaxCode.search('POS') != -1)
						{
			...
			if(nlapiGetFieldValue('custpage_ava_shipping') == 'T' && nlapiGetFieldValue('shippingcost') != null && nlapiGetFieldValue('shippingcost') > 0)
			{
				...
				if(ShipTaxCode != null && ShipTaxCode.length > 0)
				{
					if(ShipTaxCode != '-Not Taxable-')
					{
						...
						{
							if(ShipTaxCode.search('POD') != -1 || ShipTaxCode.search('POS') != -1)
							{
			...
			if(nlapiGetFieldValue('custpage_ava_handling') == 'T' && nlapiGetFieldValue('handlingcost') != null && nlapiGetFieldValue('handlingcost') > 0)
			{
				...
				if(HandlingTaxCode != null && HandlingTaxCode.length > 0)
				{
					if(HandlingTaxCode != '-Not Taxable-')
					{
						...
						{
							if(HandlingTaxCode.search('POD') != -1 || HandlingTaxCode.search('POS') != -1)
							{
...
function AVA_GetSubsidiaryInfo()
{
	...
	try
	{
		var AVA_Subsidiary = JSON.parse(nlapiGetFieldValue('custpage_ava_customsubsidiaryinfo'));
		for(var i in AVA_Subsidiary)
		{
			if(AVA_Subsidiary[i].columns['custrecord_ava_subsidiary'].internalid == subID)
			{
				var AddrFlag = AVA_Subsidiary[i].columns['custrecord_ava_iscompanyaddr'];
				
				AVA_Def_Addressee  = (AddrFlag==true) ? nlapiGetFieldValue('custbody_ava_subsidiaryaddressee')     :'Default Shipping Address';
				AVA_Def_Addr1      = (AddrFlag==true) ? nlapiGetFieldValue('custbody_ava_subsidiaryaddress1')     : nlapiGetFieldValue('custbody_ava_subsidiaryshipaddress1');
				AVA_Def_Addr2      = (AddrFlag==true) ? nlapiGetFieldValue('custbody_ava_subsidiaryaddress2')     : nlapiGetFieldValue('custbody_ava_subsidiaryshipaddress2');
				AVA_Def_City       = (AddrFlag==true) ? nlapiGetFieldValue('custbody_ava_subsidiarycity')     : nlapiGetFieldValue('custbody_ava_subsidiaryshipcity');
				AVA_Def_State      = (AddrFlag==true) ? nlapiGetFieldValue('custbody_ava_subsidiarystate')     : nlapiGetFieldValue('custbody_ava_subsidiaryshipstate');
				AVA_Def_Zip        = (AddrFlag==true) ? nlapiGetFieldValue('custbody_ava_subsidiaryzip')     : nlapiGetFieldValue('custbody_ava_subsidiaryshipzip');
				var ReturnCountryName = AVA_CheckCountryName(((AddrFlag==true)) ? nlapiGetFieldValue('custbody_ava_subsidiarycountry'): nlapiGetFieldValue('custbody_ava_subsidiaryshipcountry'));  
				AVA_Def_Country    = ReturnCountryName[1];
				AVA_SubCurrency    = nlapiGetFieldValue('custbody_ava_subsidiarycurrency');
				
				var Taxcode = AVA_Subsidiary[i].columns['custrecord_ava_subdeftaxcode'];
...
function AVA_ItemsTaxLines()
{
	...
	for(var i=0 ; i<nlapiGetLineItemCount('item') ; i++)
	{
		...
		if(AVA_LineType[i] != 'EndGroup')
		{
			...
			{
				AVA_LineNames[i]  =  'UPC:' + nlapiGetLineItemValue('item','custcol_ava_upccode',i+1).substring(0,45);
			}
			else
			{
				AVA_LineNames[i]  = (nlapiGetLineItemValue('item','custcol_ava_item',i+1) != null && nlapiGetLineItemValue('item','custcol_ava_item',i+1).length > 0) ? nlapiGetLineItemValue('item','custcol_ava_item',i+1).substring(0,49) : '';
			}
		}
		else
		{
		...
		if(i == (AVA_GroupBegin-1))
		{
			for(var k=i; k<=AVA_GroupEnd ; k++)
			{
				...
				if(AVA_LineType[k] != 'EndGroup')
				{
					...
					{
						AVA_LineNames[k]  =  'UPC:' + nlapiGetLineItemValue('item','custcol_ava_upccode',k+1).substring(0,45);
					}
					else
					{
						AVA_LineNames[k]  = (nlapiGetLineItemValue('item','custcol_ava_item',k+1) != null && nlapiGetLineItemValue('item','custcol_ava_item',k+1).length > 0) ? nlapiGetLineItemValue('item','custcol_ava_item',k+1).substring(0,49) : '';
					}
				}
				else
				{
...
function AVA_ViewTaxDetails(request, response)
{
	if(AVA_CheckService('TaxSvc') == 0 && AVA_CheckSecurity( 6 ) == 0)
	{
		if(request.getMethod() == 'GET')
		{
			...
			var security = AVA_TaxSecurity(AVA_AccountValue, AVA_LicenseKey); 
			var headers = AVA_Header(security);
			var body = AVA_GetTaxHistory(request.getParameter('doctype'),request.getParameter('doccode'),request.getParameter('subid'));
			var soapPayload = AVA_GetTaxEnvelope(headers + body);
...
function AVA_GetTaxHistory(RecordType, RecordId, SubId)
{
	var soap = null;
	soap = '\t<soap:Body>\n';
		soap += '\t\t<GetTaxHistory xmlns="http://avatax.avalara.com/services">\n';
			soap += '\t\t\t<GetTaxHistoryRequest>\n';
				soap += '\t\t\t\t<CompanyCode><![CDATA[' + SubId + ']]></CompanyCode>\n';
...
function AVA_RecordLoadSuitelet(request,response)
{
	try
	{
		...
		switch(request.getParameter('type'))
		{
			...
			/*case 'customrecord_avasubsidiaries':
				//Fetch subsidiary taxcode from custom record.
				var filter = new Array();
				filter[0] = new nlobjSearchFilter('custrecord_ava_subsidiary',null,'anyof',request.getParameter('id'));
				...
				{ 
					...
					var subRec = nlapiLoadRecord('subsidiary',request.getParameter('id')); 
					
					AVA_Def_Addressee  = (AddrFlag == 'T') ? subRec.getFieldValue('addressee')     :'Default Shipping Address';
					AVA_Def_Addr1      = (AddrFlag == 'T') ? subRec.getFieldValue('addr1')         : subRec.getFieldValue('shipaddress1');
					AVA_Def_Addr2      = (AddrFlag == 'T') ? subRec.getFieldValue('addr2')         : subRec.getFieldValue('shipaddress2');
					AVA_Def_City       = (AddrFlag == 'T') ? subRec.getFieldValue('city')          : subRec.getFieldValue('shipcity');
					AVA_Def_State      = (AddrFlag == 'T') ? subRec.getFieldValue('dropdownstate') : subRec.getFieldValue('shipstate');
					AVA_Def_Zip        = (AddrFlag == 'T') ? subRec.getFieldValue('zip')           : subRec.getFieldValue('shipzip');
					var ReturnCountryName = AVA_CheckCountryName((AddrFlag == 'T') ? subRec.getFieldValue('country') : subRec.getFieldValue('shipcountry')); 
					AVA_Def_Country    = ReturnCountryName[1];
					AVA_SubCurrency    = subRec.getFieldValue('currency');

					recordValues = AVA_DefaultTaxCode + '+' + AVA_Def_Addressee + '+' + ((AVA_Def_Addr1 != null) ? AVA_Def_Addr1 : '') + '+' + ((AVA_Def_Addr2 != null) ? AVA_Def_Addr2 : '') + '+' + ((AVA_Def_City != null) ? AVA_Def_City : '') + '+' + ((AVA_Def_State != null) ? AVA_Def_State : '') + '+' + ((AVA_Def_Zip != null) ? AVA_Def_Zip : '') + '+' + ((AVA_Def_Country != null) ? AVA_Def_Country : '') + '+' + AVA_SubCurrency;
