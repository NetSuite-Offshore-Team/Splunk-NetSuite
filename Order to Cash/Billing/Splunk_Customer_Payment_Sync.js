/*
* This script file is used to create/update Customer Payment in Cloudhub
* @ AUTHOR : Vaibhav
...
* @Version 1.2
*/
function SyncCustomerPayment(type) {
	try {
	/* Checking the type of execution. The script should run on 'Create' and 'Edit' only */
	...
	/*----Start------------Creating JSON Object to be sent to Cloudhub------------Start-----*/
			...
			var r = nlapiLoadRecord('customerpayment',nlapiGetRecordId());	
	...
	/*----Start-------------------Clearing the sync Error fields-----------------Start------*/		
	        ...
	        value[0] = 'F';
            value[1] = '';
			nlapiSubmitField('customerpayment',nlapiGetRecordId(),field,value);
			}
	/*----End-------------------Clearing the sync Error fields-----------------End------*/
            ...
            nlapiSubmitField('customerpayment',nlapiGetRecordId(),'custbody_spk_sfdc_object_id',y.getBody().toString().split('"objectId":"')[1].split('","')[0]);
			}	
	        ...
	        nlapiSubmitField('customerpayment',nlapiGetRecordId(),fields,values);	//Capturing error message in a custom field
            }
