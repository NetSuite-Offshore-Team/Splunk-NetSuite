/*
* This script file is used to create/update Customer deposit in Cloudhub
* @ AUTHOR : Vaibhav
...
* @Version 1.2
*/
function SyncCustomerDeposit(type) {
    try {
...
nlapiLogExecution('DEBUG', 'type', type);
	...
	/*----Start------------Creating JSON Object to be sent to Cloudhub------------Start-----*/
	        ...
	        var r = nlapiLoadRecord('customerdeposit',nlapiGetRecordId());	
	...
	/*----Start-------------------Clearing the sync Error fields-----------------Start------*/		
	        ...
	        value[0] = 'F';
            value[1] = '';
			nlapiSubmitField('customerdeposit',nlapiGetRecordId(),field,value);
			}
	/*----End-------------------Clearing the sync Error fields-----------------End------*/
	        ...
	        var customer = r.getFieldValue('customer');
            if ((customer == null) || (customer == '')) {
                customer = '""';
            }
            else {
                customer = '"' + sfdccustomer + '"';                
				}
			...
			var total = r.getFieldValue('payment');
            if ((total == null) || (total == '')) {
                total = '""';
            }
            ...
            var soid = r.getFieldValue('salesorder');
			if(soid != '' && soid != null)
			{
            var sfdcoppid = nlapiLookupField('salesorder',soid,'custbody_celigo_ns_sfdc_sync_id');
			sfdcoppid = '"' + sfdcoppid + '"';
			}
			else{
			var sfdcoppid = '""';
			}
            string1 = string1 + ',"oppurtunityID" : ' + sfdcoppid;
            ...
            string1 = string1 + ',"type" : "Customer Deposit"';
			...
			/*------Start-----Checking if the request is processed successfully----------Start-------*/	
                    ...
                    nlapiSubmitField('customerdeposit',nlapiGetRecordId(),'custbody_spk_sfdc_object_id',y.getBody().toString().split('"objectId":"')[1].split('","')[0]);
                    }	
			...
			/*------Start-------------If the sent request failed---------------------Start-------*/	
	                ...
	                nlapiSubmitField('customerdeposit',nlapiGetRecordId(),fields,values);	//Capturing error message in a custom field
                    }
