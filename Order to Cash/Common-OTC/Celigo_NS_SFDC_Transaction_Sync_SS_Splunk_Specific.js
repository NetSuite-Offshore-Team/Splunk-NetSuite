...
function sendToIntegrator(type, newRecord, url, array) {
	try {
		var attachPdf = 'T'; //nlapiGetContext().getSetting('SCRIPT', 'custscript_celigo_ns_sfdc_attach_pdf');
		if (attachPdf == 'T' && type.toLowerCase() != 'delete' && newRecord.getRecordType().toLowerCase() == nlapiGetRecordType().toLowerCase()) {
			var pdf = null;
			if(nlapiGetRecordType() == 'estimate' || nlapiGetRecordType() == 'invoice')
				pdf = printWithTC(nlapiGetRecordType(), nlapiGetRecordId());
			else
				pdf = nlapiPrintRecord('TRANSACTION', newRecord.getId(), 'PDF', null);
